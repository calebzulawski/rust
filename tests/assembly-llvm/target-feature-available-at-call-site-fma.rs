...
#[inline(always)]
unsafe fn maybe_fma(x: f32, y: f32, z: f32) -> f32 {
    if target_feature_available_at_call_site("fma") { x.mul_add(y, z) } else { x * y + z }
}

#[inline(always)]
unsafe fn maybe_fma_arch_intrinsic(x: f32, y: f32, z: f32) -> f32 {
    let x = _mm_set_ss(x);
    let y = _mm_set_ss(y);
    let z = _mm_set_ss(z);
...
pub unsafe fn with_fma(x: f32, y: f32, z: f32) -> f32 {
    maybe_fma(x, y, z)
}
...
// CHECK-NOT: vfmadd
#[no_mangle]
pub unsafe fn without_fma(x: f32, y: f32, z: f32) -> f32 {
    maybe_fma(x, y, z)
}

#[no_mangle]
#[target_feature(enable = "avx,fma")]
pub unsafe fn with_fma_arch_intrinsic(x: f32, y: f32, z: f32) -> f32 {
...
// CHECK-NOT: vfmadd
#[no_mangle]
pub unsafe fn without_fma_arch_intrinsic(x: f32, y: f32, z: f32) -> f32 {
    maybe_fma_arch_intrinsic(x, y, z)
}
